...
public class InstantMessagingTest {
    ...
    */
    ...
    public void isElem() {
        ...
        ArrayList spyList = new ArrayList<TextMessageTest>(0);
...
/*
        ...
        for (int i = DynamicSpyList.size(); i > 0; i--) {
            ...
            for (int j= spyList.size(); j > 0; j--) {
                if (spyList.get(j-1).getMessageText() == DynamicSpyList.get(i-1).text &&  spyList.get(j-1).time == DynamicSpyList.get(i-1).time ) {
                    isElem = true;
                }
            }
